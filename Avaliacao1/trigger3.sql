...
CREATE OR REPLACE FUNCTION fc_adiciona_operadora2()

RETURNS trigger AS $$
BEGIN
	INSERT INTO contato (id, operadora, telefone, nome, email, nivel)
	VALUES (99, 'Adicionar Nova Operadora', 99999999, 'Adicionar Nome', 'email@dominio.com', 1);
--	(SELECT OLD.*);
	RETURN NEW;
	END;
$$ LANGUAGE plpgsql;
...
ON operadoras
	FOR EACH ROW EXECUTE PROCEDURE fc_adiciona_operadora2();